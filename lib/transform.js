 ...
 * set layout
 */
Transform.prototype.setLayout = function (){
  var context = this;
  ...
  fs.readFile(context.layout, function (error, source){
    ...
    // circle event
    layout.on(EVENTS.CIRCLE, function (src){
      context.circle(src);
    });
      ...
      default:
        if (command.indexOf(DIRECTIVE.INCLUDE) === 0) {
          ...
          fs.readFile(src, function (error, source){
            ...
            // circle event
            include.on(EVENTS.CIRCLE, function (src){
              context.circle(src);
            });
 ...
 * @param src
 */
Transform.prototype.circle = function (src){
  this.emit(EVENTS.CIRCLE, src)
};
