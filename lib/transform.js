...
var assert = {
  ...
  tags: function (context){
    ...
    function assert(key){
      ...
      if (!util.array(tags)) {
        options.tags[key] = def.map(function (tags){
          return util.str4regex(tags);
        });
      } else {
    ...
    case 'directive':
      ...
      switch (commandIgnoreCase) {
        case DIRECTIVE.SLOT:
          ...
          break;
        default:
          if (commandIgnoreCase.indexOf(DIRECTIVE.INCLUDE) === 0) {
            if (data) {
            ...
            // same tags conver data
            if (context.isSameTags()) {
              context.print(match, command);
            } else {
              context.next(match);
            }
          }
