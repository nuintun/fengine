...
Transform.prototype.readLayout = function (){
  ...
  if (layout && (!nonlayout || layout.index < nonlayout.index)) {
    context.layout = context.resolve(layout[1]);
  }
  ...
  if (context.layout) {
    fs.readFile(context.layout, function (error, source){
      ...
      var layout = new Transform(context.layout, source.toString(), options);
    ...
    switch (command) {
      ...
      default:
        if (command.indexOf(DIRECTIVE.INCLUDE) === 0) {
          ...
          var include = context.resolve(command);

          fs.readFile(include, function (error, source){
            if (error) {
              return context.next();
            }

            var child = new Transform(include, source.toString(), options);
...
Transform.prototype.next = function (){
  ...
  if (!context.exec()) {
    ...
    !context.layout && context.emit('end');
  }
};
