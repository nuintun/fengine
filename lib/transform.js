...
function Transform(source, options){
  ...
  context.src = path.resolve(CWD, options.src);
...
Transform.prototype.layout = function (){
  var context = this;
  var layout = context.regexp.layout.exec(context.source);
  var nonlayout = context.regexp.nonlayout.exec(context.source);
  var defLayout = util.fn(context.options.layout)
    ? context.options.layout(context.src)
    : context.options.layout;

  defLayout = util.string(defLayout) ? defLayout : null;

  if (defLayout) {
    if (!path.isAbsolute(defLayout)) {
      defLayout = path.join(context.dirname, defLayout);
    }
  }
  ...
  if (defLayout
    && ((!layout && !nonlayout)
    || (layout && (!nonlayout
    || layout.index < nonlayout.index)))) {
    console.log(defLayout);
  } else {
...
Transform.prototype.next = function (){
  ...
  if (!context.exec()) {
    context.emit('data', context.source.substring(context.index));
    context.emit('end');
  }
};
