  ...
  for (var callback in dataElements) {
    ...
    switch (util.type(start)) {
      ...
      case 'regexp':
        ...
        valid = start && (start = start[start.length - 1].length);
        break;
      case 'function':
        start = start(context.substring);
        start = util.string(start)
          ? start.length
          : util.number(start) && !util.nan(start) && !util.infinity(start)
          ? Math.max(0, start)
          : -1;
  ...
  if (valid) {
    ...
    switch (util.type(end)) {
      ...
      case 'function':
          ...
          ? end.length
          : util.number(end) && !util.nan(end) && !util.infinity(end)
          ? Math.max(0, end)
          : 0;
        break;
      default:
    ...
    switch (util.type(data)) {
      case 'regexp':
        ...
        data = data && data.length ? data[data.length - 1] : param;
        break;
      case 'function':
